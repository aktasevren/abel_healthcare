...
function getSubProductsData(): SubProduct[] {
  ...
  return lines.slice(1).map((line) => {
    ...
    if (values.length < 5 || values[1] !== '14') return null;
    return {
      product_id: values[0],
...
const OtoskopPage: React.FC = () => {
  const subProducts = getSubProductsData();

  return (
    <div className="sub-products-container">
      <h1 className="sub-products-title">Otoskop</h1>
      <nav className="breadcrumb">
        <Link href="/">Anasayfa</Link> &gt; <Link href="/products">Ürünler</Link> &gt; <span>Otoskop</span>
      </nav>
