...
function getSubProductsData(): SubProduct[] {
  ...
  return lines.slice(1).map((line) => {
    ...
    if (values.length < 5 || values[1] !== '6') return null;
    return {
      product_id: values[0],
...
const MedikalUrunPage: React.FC = () => {
  const subProducts = getSubProductsData();

  return (
    <div className="sub-products-container">
      <h1 className="sub-products-title">Yeni Doğan Grubu ve Yedek Parçaları</h1>
      <nav className="breadcrumb">
        <Link href="/">Anasayfa</Link> &gt; <Link href="/products">Ürünler</Link> &gt; <span>Yeni Doğan Grubu</span>
      </nav>
...
export default MedikalUrunPage; 