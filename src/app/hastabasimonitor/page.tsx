...
function getSubProductsData(): SubProduct[] {
  ...
  return products
    .filter((product: any) => product.group_id === '3')
    .map((product: any) => ({
...
const HastabasiMonitorPage: React.FC = () => {
  const subProducts = getSubProductsData();

  return (
    <div className="sub-products-container">
      <h1 className="sub-products-title">Hastabaşı Monitor Yedek Parça ve Aksesuar</h1>
      <nav className="breadcrumb">
        <Link href="/">Anasayfa</Link> &gt; <Link href="/products">Ürünler</Link> &gt; <span>Hastabaşı Monitor</span>
      </nav>
...
export default HastabasiMonitorPage; 