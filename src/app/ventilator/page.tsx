...
function getSubProductsData(): SubProduct[] {
  ...
  return products
    .filter((product: any) => product.group_id === '1')
    .map((product: any) => ({
...
const VentilatorPage: React.FC = () => {
  const subProducts = getSubProductsData();

  return (
    <div className="sub-products-container">
      <h1 className="sub-products-title">Ventilatör Yedek Parça ve Aksesuar</h1>
      <nav className="breadcrumb">
        <Link href="/">Anasayfa</Link> &gt; <Link href="/products">Ürünler</Link> &gt; <span>Ventilatör</span>
      </nav>
