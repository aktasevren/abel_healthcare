...
function getSubProductsData(): SubProduct[] {
  ...
  return products
    .filter((product: any) => product.group_id === '18')
    .map((product: any) => ({
...
const MedikalAmpulPage: React.FC = () => {
  const subProducts = getSubProductsData();

  return (
    <div className="sub-products-container">
      <h1 className="sub-products-title">Medikal Ampul</h1>
      <nav className="breadcrumb">
        <Link href="/">Anasayfa</Link> &gt; <Link href="/products">Ürünler</Link> &gt; <span>Medikal Ampul</span>
      </nav>
...
export default MedikalAmpulPage; 