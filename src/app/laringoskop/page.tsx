...
function getSubProductsData(): SubProduct[] {
  ...
  return products
    .filter((product: any) => product.group_id === '13')
    .map((product: any) => ({
...
const LaringoskopPage: React.FC = () => {
  const subProducts = getSubProductsData();

  return (
    <div className="sub-products-container">
      <h1 className="sub-products-title">Laringoskop Setleri</h1>
      <nav className="breadcrumb">
        <Link href="/">Anasayfa</Link> &gt; <Link href="/products">Ürünler</Link> &gt; <span>Laringoskop</span>
      </nav>
...
export default LaringoskopPage; 