...
function getSubProductsData(): SubProduct[] {
  ...
  return products
    .filter((product: any) => product.group_id === '8')
    .map((product: any) => ({
...
const VeterinerPage: React.FC = () => {
  const subProducts = getSubProductsData();

  return (
    <div className="sub-products-container">
      <h1 className="sub-products-title">Veteriner Grubu</h1>
      <nav className="breadcrumb">
        <Link href="/">Anasayfa</Link> &gt; <Link href="/products">Ürünler</Link> &gt; <span>Veteriner</span>
      </nav>
