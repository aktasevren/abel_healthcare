...
function getSubProductsData(): SubProduct[] {
  ...
  return lines.slice(1).map((line) => {
    ...
    if (values.length < 5 || values[1] !== '5') return null;
    return {
      product_id: values[0],
...
const MedikalAspiratorPage: React.FC = () => {
  const subProducts = getSubProductsData();

  return (
    <div className="sub-products-container">
      <h1 className="sub-products-title">Cerrahi ve Portable Aspiratörler</h1>
      <nav className="breadcrumb">
        <Link href="/">Anasayfa</Link> &gt; <Link href="/products">Ürünler</Link> &gt; <span>Cerrahi ve Portable Aspiratörler</span>
      </nav>
...
export default MedikalAspiratorPage; 